...
def query_handler_bs(query):
    ...
    for stored_name, (data, source_info) in store_bs.items():
        if KeyExpr.intersect(query.key_selector, stored_name):
            ...
            query.reply(sample)
    if flag == 0:
        query.reply(Sample(key_expr="sdn/zone3/BS/*", payload="None".encode()))
...
store = {}
sub = session.subscribe("sdn/zone3/hosts/**", listener, reliability=Reliability.Reliable, mode=SubMode.Push)
queryable = session.queryable("sdn/zone3/hosts/**", STORAGE, query_handler)


store_bs = {}
sub_bs = session.subscribe("sdn/zone3/BS/**", listener_bs, reliability=Reliability.Reliable, mode=SubMode.Push)
queryable_bs = session.queryable("sdn/zone3/BS/**", STORAGE, query_handler_bs)
...
border_switch = [3]
session.put("sdn/zone3/BS/2",json.dumps({"via":"3","from":"3"}))
border_gw = {2:3} #to zone %d use dpid %d
flows = {}
flag=0
zone=3
...
class Controllerz1(app_manager.RyuApp):
    ...
    def update_known_hosts(self):
        ...
        for mac in known_hosts:
            session.put(f"sdn/zone3/hosts/{mac}",json.dumps(known_hosts[mac]))
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst in net and src in net:
            ...
            if src in known_hosts:
                path=nx.shortest_path(net,dpid,dst,weight='weight')
                #print(f"{src} -> {dst} use path {path}")
                next=path[path.index(dpid)+1]
                out_port=net[dpid][next]['port']
            else:
            ...
            if dst in known_hosts:
                if known_hosts[dst]["zone"] not in border_gw:
                    ...
                    session.put(f"sdn/zone3/BS/{known_hosts[dst]['zone']}",json.dumps({"via":border_gw[int(r["from"])],"from":r["from"]}))
